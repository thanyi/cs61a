...
def balanced(m):
    ...
    if is_planet(m):
        ...
        if length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m))):
            ...
            return balanced(end(left(m))) == True and balanced(end(right(m))) == True
        else:
            return False
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if is_leaf(t) and label(t) == 'loki'  :
        ...
        return tree(label(t), [replace_loki_at_leaf(item , lokis_replacement) for item in branches(t)])
...
def sub_interval(x, y):
    ...
    _nagative_y = interval(-upper_bound(y),-lower_bound(y))
    return add_interval(x,_nagative_y)
