def flatten(s):
    ...
    def flatten_helper(sth):       # 对这种类型的递归，重新定义一个函数来将res进行完整的修改
        if type(sth) != list:
            res.append(sth)
        else:
            for item in sth:
...
def my_filter(pred, seq):
    ...
    [1, 2, 3, 4, 5]
    """
    return [item for item in seq if pred(item) == True]
...
def my_reduce(combiner, seq):
    ...
    # cnt = 1
    # while cnt < len(seq):
    #     res = combiner(res,seq[cnt]) 
    #     cnt +=1
    for item in seq[1:]:
        res = combiner(res,item) 
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_a_la = get_lat(city=city_a)
    city_b_la = get_lat(city=city_b)

    city_a_lon = get_lon(city_a)
    city_b_lon = get_lon(city_b)

    return sqrt((city_a_la-city_b_la)**2+(city_a_lon-city_b_lon)**2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_c_a = distance(city_c,city_a)
    distance_c_b = distance(city_c,city_b)

    if distance_c_a < distance_c_b:
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [[item,fn(item)] for item in seq[lower:upper+1] if lower <= fn(item) <= upper ]
